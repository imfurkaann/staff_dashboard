        ...
        <div className="bg-white border border-slate-300 rounded-3xl p-6 shadow-sm min-h-[360px]">
          ...
          {activeTab === 'visitors' && (
            <div className="space-y-4 animate-fadeIn">
              ...
              {visitorRecordsLoading ? <div className="p-5 text-center bg-slate-50 rounded-2xl border border-slate-200 text-slate-500 text-xs font-bold">Ziyaretçi kayıtları yükleniyor...</div> : visitorRecords.length > 0 ? (
                <div className="border border-slate-300 rounded-2xl overflow-hidden shadow-sm bg-white">
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-100/80 border-b border-slate-300 font-extrabold text-slate-800">
                        <th className="py-2.5 px-3 border-r border-slate-200">Ziyaretçi</th><th className="py-2.5 px-3 border-r border-slate-200">Ziyaret Bilgisi</th><th className="py-2.5 px-3 border-r border-slate-200 w-44">Giriş Tarihi & Saati</th><th className="py-2.5 px-3 border-r border-slate-200 w-44">Çıkış Tarihi & Saati</th><th className="py-2.5 px-3">Durum</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {visitorRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                          <td className="py-2 px-3 font-extrabold text-slate-900 border-r border-slate-200">
                            <div className="flex items-center gap-2"><Users className="w-3.5 h-3.5 text-[#1e3a8a] shrink-0" /><span>{record.fullName}</span></div><div className="mt-1 text-[10px] font-semibold text-slate-500">{record.phone || 'Telefon yok'}</div>
                          </td>
                          <td className="py-2 px-3 border-r border-slate-200"><div className="font-bold text-slate-800">{record.purpose || '-'}</div><div className="mt-1 text-[10px] font-semibold text-slate-500">{record.company || 'Bireysel'}{record.vehiclePlate ? ` • ${record.vehiclePlate}` : ''}</div></td>
                          ...
                          <td className="py-2 px-3"><span className={`inline-flex rounded-lg border px-2 py-1 text-[10px] font-extrabold ${record.status === 'INSIDE' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-600'}`}>{record.status === 'INSIDE' ? 'İÇERİDE' : 'ÇIKIŞ YAPTI'}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="p-4 text-center bg-slate-50 rounded-2xl border border-slate-200 text-slate-500 text-xs font-bold">
                  Bu personel için henüz bir ziyaretçi kaydı bulunmamaktadır.
                </div>
              )}
