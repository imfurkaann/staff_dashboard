...
export const DashboardView: React.FC<DashboardViewProps> = ({ currentUser, onNavigateTo }) => {
  ...
  const cards = [
    { label: 'Toplam personel', value: summary.totalEmployees, icon: Users },
    { label: 'Lojmanda kalan', value: summary.residentEmployees, icon: Users },
    { label: 'Atama bekleyen', value: summary.pendingEmployees, icon: UserPlus },
    { label: 'Açık arıza', value: summary.openMaintenance, icon: Wrench },
  ];
  ...
  return <div className="space-y-6">
    <header className="bg-white border border-slate-300 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
      ...
      <button onClick={() => onNavigateTo('employees')} className="px-4 py-3 rounded-xl bg-[#1e3a8a] text-white text-sm font-bold flex items-center gap-2"><UserPlus className="w-4 h-4"/>Yeni personel</button>
    </header>
    <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4" aria-label="Özet bilgiler">{cards.map(({ label, value, icon: Icon }) => <div key={label} className="bg-white border border-slate-300 rounded-2xl p-5 shadow-sm"><div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-600">{label}</span><Icon className="w-5 h-5 text-[#1e3a8a]"/></div><p className="text-3xl font-black mt-3 text-slate-900">{value}</p></div>)}</section>
