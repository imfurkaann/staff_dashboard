...
export class PushService {
  ...
  static getPublicKey() {
    if (!config.push.publicKey) throw new AppError('Push bildirimleri sunucuda yapılandırılmamış.', 503);
  ...
  static async subscribe(userId: string, input: SubscriptionInput, userAgent?: string) {
    ...
    return prisma.pushSubscription.upsert({
      where: { endpoint },
      create: { userId, endpoint, p256dh, auth, userAgent: userAgent?.slice(0, 500) },
      update: { userId, p256dh, auth, userAgent: userAgent?.slice(0, 500) },
    });
  }

  static async unsubscribe(userId: string, endpoint: unknown) {
    if (typeof endpoint !== 'string') throw new AppError('Geçersiz abonelik adresi.', 400);
